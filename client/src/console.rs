...
use egui::{Align, Context, Event, EventFilter, Id, Key, Modifiers, TextEdit, Ui};
...
pub struct ConsoleWindow {
    ...
    pub tab_nth: usize,
    pub tab_quote: char,
    pub tab_command_table: Vec<String>,
}
...
impl ConsoleWindow {
    ...
    fn cursor_at_end(&self) -> CCursorRange {
        CCursorRange::one(egui::text::CCursor::new(self.text.chars().count()))
    }
    fn cursor_at(&self, loc: usize) -> CCursorRange {
        if loc >= self.text.chars().count() {
            return self.cursor_at_end();
        }
        CCursorRange::one(egui::text::CCursor::new(loc))
    }

    fn ui(&mut self, ui: &mut Ui) {
        egui::ScrollArea::both().show(ui, |ui| {
            ui.add_sized(ui.available_size(), |ui: &mut Ui| {
                let widget = TextEdit::multiline(&mut self.text)
    ...
    fn handle_key(
        ...
        let return_value = match (modifiers, key) {
            ...
            (Modifiers::NONE, Key::ArrowLeft) | (Modifiers::NONE, Key::Backspace) => {
                // in either mode do not allow motion (or deleting) into prompt
